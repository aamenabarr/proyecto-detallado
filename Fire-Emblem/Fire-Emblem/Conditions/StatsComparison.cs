...
public class StatsComparison : Condition
{
    ...
    public StatsComparison(
        Unit unit1, string stat1, string symbol, Unit unit2, string stat2, int extraValue = 0, bool flight = false)
    {
        _unit1 = unit1; 
        _unit2 = unit2;
        _stat1 = stat1; 
    ...
    {
        if (_flight) return MetFlightConditions();
        if (_stat2 == "") return MetSumOfStatsCondition();
    ...
    private bool MetFlightConditions()
    {
        return _unit1.InitialStats[_stat1] >= _unit2.InitialStats[_stat2] + _extraValue;
    }

    private bool MetSumOfStatsCondition()
    {
